...
PROCESS_THREAD(state_change, ev, data) {
  ...
  while(1) {
    ...
    //     }
    // }
    // if ((is_light_diff() && state == INTERIM_STATE)) {
    //     printf("detected light\n");
    //     printf("INTERIM -> BUZZ\n");
    //     state = BUZZ_STATE;
    //     buzzer_start(2069);
    ...
    // if(state == BUZZ_STATE && etimer_expired(&buzzer_timer)) {
    //   buzzer_stop();
    //   etimer_set(&wait_timer, CLOCK_SECOND/4);
    //   PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&wait_timer));
    //   if (is_light_diff()) {
    ...
    //     printf("BUZZ -> WAIT\n");
    //   // print_statistics();
    //     etimer_set(&wait_timer, CLOCK_SECOND*3); // wait for 3 seconds
    //     PROCESS_WAIT_EVENT_UNTIL(etimer_expired(&wait_timer));
    //     etimer_set(&wait_timer, (CLOCK_SECOND/4)*3); // wait for 3/4 seconds
    //   }
    // }

    // if(state == WAIT_STATE && etimer_expired(&wait_timer) ) {
    //   printf("WAIT -> BUZZ\n");
    //   // print_statistics();
